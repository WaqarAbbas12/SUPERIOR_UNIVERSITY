...
loaded_model = load_model("sentiment_model_tfidf.h5")
loaded_vectorizer = joblib.load("tfidf_vectorizer.pkl")


def predict_loaded_model(text):
    cleaned_text = preprocess_text(text)
    vectorized = loaded_vectorizer.transform([cleaned_text]).toarray()
    if vectorized.shape[1] < 5000:
        vectorized = np.pad(
            vectorized, [(0, 0), (0, 5000 - vectorized.shape[1])], mode="constant"
        )
    vectorized = vectorized.reshape(1, 5000, 1)
    prediction = loaded_model.predict(vectorized)
    return inverse_label_map[np.argmax(prediction)]
...
for text in test_samples:
    print(f"Text: {text}")
    print(f"Prediction: {predict_loaded_model(text)}\n")
